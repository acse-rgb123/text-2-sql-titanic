...
def generate_sql_prompt(query, relevant_schema, relevant_gold_sql=None):
    ...
    for col, _ in relevant_schema:
        if col and "Table" in col:  # Ensure col is not None
            table_name = col.split(",")[0].replace("Table", "").strip()
            break

    # Ensure we handle only columns with proper format
    relevant_cols = [col.split(",")[1].replace("Column", "").strip() for col, _ in relevant_schema if col and "," in col]
    ...
    input_text = (
        ...
        f"Make sure to not include any comments, just the SQL code."
    )
